...
```bash
scs --version   # 0.2.1 on the v0.2.1 release
```
...
tar -xzf specticus-0.2.1-linux-x86_64.tar.gz
mkdir -p "$HOME/.local"
cp -R specticus-0.2.1-linux-x86_64/bin specticus-0.2.1-linux-x86_64/libexec "$HOME/.local/"
...
specticus uses Semantic Versioning. Current release: **0.2.1** (`v0.2.1`).
...
(should match the installed release; **0.2.1** after this tag).
