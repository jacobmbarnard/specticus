...
struct Init: ParsableCommand {
    ...
    func run() throws {
        ...
        print("  • Style pack `\(resolved.id)` — \(resolved.summary)")
        print("  • title.yml, welcome-template.md, style.css")
        print("  • Vertical section folders from the pack (#139/#141); nest feature folders as needed")
        print("  • .specticus/config.yml (doc.style=\(resolved.id)) + layout.yml (#140/#141)")
        print("")
        print("Next steps:")
        if targetDir != "." {
            print("  cd \(targetDir)")
